	...
	$.fn.selectstyle = function(option){
		...
		this.each(function(){
			...
			$this.find('option').each(function (e) {
				...
				html_op += '>';
				if(image != null){
					html_op += '<div class="ssli_image"></div>';
				}
			...
			'<div class="selectstyle ss_dib '+setting.theme+'" style="width:'+parseInt(setting.width)+'px;">'+
				'<div id="select_style" class="ss_button" style=" width:'+parseInt(setting.width)+'px;'+style+'">'+
					'<div class="ss_dib ss_text" id="select_style_text" >'+placeholder+'</div>'+
		...
		$("body").delegate("ul#select_style_ul li", "click", function(e) {
			...
			$(this).parents('ul#select_style_ul').parent('div').find('div#select_style_text').html(txt).addClass("textOn").next().addClass("textOn");
