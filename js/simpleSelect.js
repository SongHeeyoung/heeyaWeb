        ...
        function select(term) {
            ...
            for (var i = 0; i < $options.length; i++) {
                if ($options[i].value === term) {
                    ...
                    $container.find('.selected').text(term).addClass('pushTxt');
                    break;
                }
            }
        }
...
var options2 = {
    ...
    notFoundMessage: 'No Results.',
    defaultSelected: '지점선택'
};
$('#selectCompany').simpleSelect(options);
$('#selectPoint').simpleSelect(options2);
